...
# Convenience: actually return the final x (state) via callback capture.
def sample_unipc_x(model, noise, sigmas, extra_args=None, disable=False, variant='bh1'):
    last_x = {'x': None}
    def _cb(d):
        last_x['x'] = d['x']
    FlowMatchUniPC(model, extra_args=extra_args, variant=variant).sample(noise, sigmas=sigmas, callback=_cb, disable_pbar=disable)
    return last_x['x']
